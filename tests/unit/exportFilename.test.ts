...
describe('buildExportFilename', () => {
  it('builds a filename with template id and a timestamp', () => {
    ...
    expect(buildExportFilename('wall-led', date)).toBe(
      'signage-canvas_wall-led_20260105-090807.png',
    );
  });

  it('pads single-digit date/time components', () => {
    const date = new Date(2026, 8, 1, 1, 2, 3);
    expect(buildExportFilename('stand-display', date)).toBe(
      'signage-canvas_stand-display_20260901-010203.png',
    );
  });
